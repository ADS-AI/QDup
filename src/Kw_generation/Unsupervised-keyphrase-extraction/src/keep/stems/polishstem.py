...
def removeSuffix(word):
    ...
    if word.find("^") > -1:
        if word[word.find("^") + 1] == "i":
            if (word[word.find("^") - 1] in vowels) and (
                oneSylab(word[0 : word.find("^")])
            ):
...
def applyAlternation(word, alternation):
    ...
    if word.find("^") > -1:
        a = (
            word[0 : (word.find("^") - len(alternation))]
            + target
            + word[word.find("^") : len(word)]
        )
    else:
            ...
            for i in range(len(train)):
                ...
                if s_index > 0:
                    if s_index - 2 < len(target[i]):
                        if (
                            train[i][s_index - 2].islower()
                            and target[i][s_index - 2].islower()
                            ...
                            if train[i][s_index - 2] != target[i][s_index - 2]:
                                if train[i][s_index - 3] != target[i][s_index - 3]:
                                    ...
                                    ast.append(
                                        (
                                            train[i][(s_index - 3) : (s_index - 1)],
                                            target[i][(s_index - 3) : (s_index - 1)],
                                        )
                                    )
                                else:
                                    a.update(
                                        {train[i][s_index - 2]: target[i][s_index - 2]}
                                    )
                                    ast.append(
                                        (train[i][s_index - 2], target[i][s_index - 2])
                                    )
                                ...
                                a.update(
                                    {
                                        train[i][(s_index - 3) : (s_index - 1)]: target[
                                            i
                                        ][(s_index - 3) : (s_index - 1)]
                                    }
                                )
                    ...
                    if len(train[i]) == len(target[i]):
                        if train[i][len(train[i]) - 2] != target[i][len(train[i]) - 2]:
                            morph_changes.update(
                                {
                                    train[i][len(train[i]) - 2]: target[i][
                                        len(train[i]) - 2
                                    ]
                                }
                            )
        ...
        for i in range(len(test)):
            if not noSuffix(test[i]):
                for e in endings:
                    ...
                    if s_index > 0:
                        if (
                            test[i][(s_index - 3) : (s_index - 1)]
                            in self.alternation[e]
                        ):
                            test[i] = applyAlternation(
                                test[i],
                                self.alternation[e][
                                    test[i][(s_index - 3) : (s_index - 1)]
                                ],
