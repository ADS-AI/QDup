...
class KeywordExtractor:
    ...
    def run(
        ...
        if (
            method == "CoTagRank"
            or method == "CoTagRankWindow"
            or method == "TopicCSRankLDA"
        ):
            phrases = self.phrase_extractor.run(doc_text, lists)
            # print("phrases", phrases)
            text_embedding, phrase_embeddings = self.embed.run(
                doc_text, text, phrases, lda_model, dictionary, False
            )
            if len(phrases) > 0:
                ranked_phrases, phrases_with_positions = self.rank.run(
                    doc_text, phrases, text_embedding, phrase_embeddings, highlight
                )
            else:
            ...
            if expand:
                ...
                similar_concepts, summaries = concept_expansion.expand_concepts(
                    concepts
                )
                ...
                if len(similar_concepts) != 0:
                    concepts = [
                        (concept.lower(), None, None) for concept in similar_concepts
                    ]

                    # concepts = [(concept,score) for score, concept in ranked_phrases]
                    # similar_concepts = [concept.lower() for concept in similar_concepts]
                    # concepts.extend(similar_concepts)
                    text_embedding, expanded_phrases_embeddings = self.embed.run(
                        doc_text, text, concepts, lda_model, dictionary, expand
                    )
                    ...
                    ranked_expanded_phrases, phrases_with_positions = self.rank.run(
                        doc_text,
                        concepts,
                        text_embedding,
                    ...
                    for relevance, phrase in ranked_phrases:
                        if not phrase in final_list_candidates:
                            lemmatized_phrase = " ".join(
                                wnl.lemmatize(word) for word in phrase.split(" ")
                            )
        ...
        if lda_model != None:
            phrases = self.phrase_extractor.run(doc_text, lists)
            text_embedding, phrase_embeddings = self.embed.run(
                doc_text, text, phrases, lda_model, dictionary
            )

        else:
            phrases = self.phrase_extractor.run(doc_text, lists=lists)
            text_embedding, phrase_embeddings = self.embed.run(
                doc_text, phrases, method
            )
        if method == "TopicCSRank" or method == "CoTagRankSentenceUSE":
            ranked_phrases, phrase_relevance = self.rank.run(
                doc_text, phrases, text_embedding, phrase_embeddings
            )
        else:
            ranked_phrases, phrase_relevance, phrase_aliases = self.rank.run(
                doc_text, phrases, text_embedding, phrase_embeddings
            )
        # print("ranked_phrases", ranked_phrases)
        if expand:
            ...
            if method == "CoTagRankSentenceUSE":
                ...
                keywords = [
                    (keyword)
                    for (keyword, _, _), score in zip(ranked_phrases, phrase_relevance)
                    if keyword
                ]
            ...
            if len(similar_concepts) != 0:
                ...
                if lda_model != None:
                    ...
                    text_embedding, expanded_phrases_embeddings = self.embed.run(
                        doc_text, text, expanded_concepts, lda_model, dictionary
                    )
                elif (
                    method == "EmbedRankSentenceBERT"
                    or method == "CoTagRanks2v"
                    or method == "TopicCSRank"
                ):
                    expanded_concepts = [
                        (concept.lower(), None, None) for concept in similar_concepts
                    ]
                    text_embedding, expanded_phrases_embeddings = self.embed.run(
                        doc_text, expanded_concepts
                    )
                ...
                if method == "CoTagRankSentenceUSE" or method == "TopicCSRank":
                    ranked_expanded_phrases, phrase_relevance_expanded = self.rank.run(
                        doc_text,
                        expanded_concepts,
                        text_embedding,
                        expanded_phrases_embeddings,
                    ...
                    (
                        ranked_expanded_phrases,
                        phrase_relevance_expanded,
                        phrase_aliases,
                    ) = self.rank.run(
                        doc_text,
                        expanded_concepts,
                ...
                if method == "CoTagRankSentenceUSE" or method == "TopicCSRank":
                    ...
                    ranked_expanded_phrases = [
                        (ranked_phrase, None, None)
                        for (ranked_phrase, _, _) in ranked_expanded_phrases
                    ]
                # print("ranked_phrases*************", ranked_phrases)
                ...
                if (
                    method != "CoTagRanks2v"
                    and method != "CoTagRankSentenceUSE"
                    and method != "TopicCSRank"
                ):
