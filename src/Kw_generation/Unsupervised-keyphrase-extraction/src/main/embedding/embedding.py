...
class UseSentenceEmbedding(Embedding):
    ...
    def get_tokenized_sents_embeddings_USE(self, sents, expand=False):
        ...
        vectors_USE = self.session.run(
            self.embedded_text, feed_dict={self.text_input: sents}
        )
...
class TopicSentenceLDA(Embedding):
    def __init__(self, encoder):
        ...
        self.encoder = SentenceTransformer(
            dir_path + "/../../data/bi_encoder_sentence_triplets"
        )
            ...
            for phrase in phrases:
                ...
                for word in phrase.split(" "):
                    if (word) in dictionary:
                        word_vectors.append(
                            [
                                distributions[k][dictionary.index((word))]
                                for k in range(K)
                            ]
                        )
                    else:
                ...
                if word_vectors:
                    phrase_vectors.append(
                        [sum(word_list) for word_list in zip(*word_vectors)]
                    )
    ...
    def run(self, doc_text, text, phrases, lda_model, dictionary, expand=False):
        ...
        word_vectors = self.fetch_word_vector_rep(
            joint_corpus[1:], lemmatizer, dictionary, K, distributions
        )
...
class TopicalSentenceEmbedding(Embedding):
    ...
    def run(self, text, phrases, method=None):
        ...
        embeddings = self.encoder.encode(
            [text for text, _, _ in [(text, 0, -1)] + phrases]
        )
...
class Sent2Vec(Embedding):
    ...
    def run(self, text, phrases, method=None):
        if method == "EmbedRank":
            embeddings = self.model.embed_sentences(
                [text for text in [(text)] + phrases]
            )
        else:
            embeddings = self.model.embed_sentences(
                [text for text, _, _ in [(text, 0, -1)] + phrases]
