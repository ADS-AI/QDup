...
class CoTagRanks2v:
    ...
    def run(self, text, phrases, text_emb, phrase_embs):
        top_phrases, relevance, aliases = self.graphRank(
            text_emb,
            phrases,
            phrase_embs,
            self.mmr_beta,
    ...
    def graphRank(
        ...
        document_relevance = (
            (text_sims_norm[unselected_phrase_indices]).squeeze(axis=1).tolist()
        )
        ...
        graph.add_weighted_edges_from(
            [
                (
                    ...
                    (
                        (
                            np.dot(phrase_to_embedding[v[0]], phrase_to_embedding[u[0]])
                            / (
                                (
                                    np.linalg.norm(phrase_to_embedding[v[0]])
                                    * np.linalg.norm(phrase_to_embedding[u[0]])
                                )
                                + 1e-07
                            )
                        )
                ...
                for v in top_phrases
                for u in top_phrases
                if u != v
            ]
        )
        ...
        try:
            pr = nx.pagerank(
                graph,
                personalization=relevance_dict,
                alpha=0.85,
                tol=0.0001,
                weight="weight",
            )
        ...
        concepts = sorted([(b, a.lstrip()) for a, b in pr.items()], reverse=True)[
            :top_n
        ]
    ...
    def standardize_normalize_cosine_similarities(cosine_similarities):
        ...
        cosine_sims_norm = 0.5 + (cosine_sims_norm - np.mean(cosine_sims_norm)) / (
            np.std(cosine_sims_norm) + 1e-07
        )
