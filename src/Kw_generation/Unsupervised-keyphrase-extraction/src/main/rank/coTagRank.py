...
class CoTagRank:
    ...
    def __init__(
        self, emb_method="NAIVE", mmr_beta=0.55, top_n=15, alias_threshold=0.8
    ):
        self.top_n = top_n
    ...
    def run(self, text, phrases, text_emb, phrase_embs, highlight=False):
        top_phrases, aliases = self.RankPhrases(
            text_emb, phrases, phrase_embs, self.top_n, self.alias_threshold, highlight
        )
    ...
    def RankPhrases(
        ...
        if highlight:
            ...
            phrases_selected = [
                phrases[phrases_only.index(phrase[1])] for phrase in concepts
            ]
