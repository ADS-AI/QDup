...
def get_phrases(pr, graph, text, ng=2, pl2=0.6, pl3=0.3, with_tag=True):
    ...
    for n in range(2, ng + 1):
        for ngram in ngrams(tokens, n):
            ...
            if all(is_word(token) for token in ngram):
                ...
                if (
                    head in edges
                    and tail in edges[head]
                    and pos_tag([ngram[-1]])[0][1] != "JJ"
                ):
    ...
    out_sorted = sorted(
        sorted_phrases + sorted_word, key=lambda d: (d[1], d[0]), reverse=True
    )
    return out_sorted
...
def get_phrases_new(pr, graph, text, ng=2, pl2=0.6, pl3=0.3, with_tag=True):
    ...
    for n in range(2, ng + 1):
        for ngram in ngrams(tokens, n):
            if (
                pos_tag([ngram[0]])[0][1] in noun_tags + adj_tags
                and pos_tag([ngram[-1]])[0][1] in noun_tags
            ):
