...
class EmbedRank:
    ...
    def __init__(
        self, emb_method="NAIVE", mmr_beta=0.55, top_n=10, alias_threshold=0.8
    ):
    ...
    def run(self, text, phrases, text_emb, phrase_embs):
        top_phrases, relevance, aliases = self.mmr(
            text_emb,
            phrases,
            phrase_embs,
            self.mmr_beta,
    ...
    def mmr(
        self, text_emb, phrases, phrase_embs, beta=0.55, top_n=15, alias_threshold=0.8
    ):
        ...
        valid_candidates_mask = ~np.all(
            phrase_embs == 0, axis=1
        )  # Only candidates which are not unknown.
        ...
        for _ in range(min(len(phrases), top_n) - 1):
            unselected_phrase_distances_to_text = text_sims_norm[
                unselected_phrase_indices, :
            ]
            unselected_phrase_distances_pairwise = phrase_sims_norm[
                unselected_phrase_indices
            ][:, selected_phrase_indices]

            # if dimension of phrases distances is 1 we add additional axis to the end
            if unselected_phrase_distances_pairwise.ndim == 1:
                unselected_phrase_distances_pairwise = np.expand_dims(
                    unselected_phrase_distances_pairwise, axis=1
                )

            # find new candidate with
            idx = int(
                np.argmax(
                    beta * unselected_phrase_distances_to_text
                    - (1 - beta)
                    * np.max(unselected_phrase_distances_pairwise, axis=1).reshape(
                        -1, 1
                    )
                )
            )
        ...
        relevance = self.max_normalize_cosine_similarities(
            text_sims[selected_phrase_indices]
        ).tolist()
        aliases_phrases = self.get_alias_phrases(
            phrase_sims[selected_phrase_indices, :], phrases, alias_threshold
        )
