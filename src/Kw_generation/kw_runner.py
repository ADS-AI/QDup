...
def get_extracted_kw():
    curr_dir = os.path.dirname(__file__)
    xtr_txt_file = os.path.join(curr_dir, 'Unsupervised-keyphrase-extraction', 'src', 'data', 'Keywords','EmbedRankSentenceBERT','EmJacc', '1')
    res = ''
...
def preprocess_keywords(keys):
  ...
  ans = []
  doc = nlp(keys)
  keys = " ".join([token.lemma_.strip("_") if len(token.lemma_) > 1 or (token.lemma_ in single_letter_elems) else "" for token in doc])
  sp = keys.split()
